...
export function publicMetadata(title: string, description: string, path: string): Metadata {
  return { title, description, alternates: { canonical: `${publicOrigin()}${path}` },
    ...
    openGraph: { title: `${title} | GG Summer`, description, type: 'website', locale: 'th_TH', url: `${publicOrigin()}${path}`, images: [{ url: `${publicOrigin()}${HERO_IMAGE}`, width: 1672, height: 941, alt: 'GG Summer — ภาพประกอบแนวคิด' }] },
  };
}
